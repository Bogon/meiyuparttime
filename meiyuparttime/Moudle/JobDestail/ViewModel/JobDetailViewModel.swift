         ...
         ))
    {
        ...
        let headerRefreshData = input.headerRefresh
            ...
            .flatMapLatest { // 也可考虑使用flatMapFirst
                dependency.networkService.jobType(detailID: input.detailID)
            }
...
class JobDetailNetworkService {
    private let provider = MoyaProvider<JobDetailAPI>.init(endpointClosure: MoyaProvider<JobDetailAPI>.defaultEndpointMapping, requestClosure: MoyaProvider<JobDetailAPI>.defaultRequestMapping, stubClosure: MoyaProvider<JobDetailAPI>.neverStub, callbackQueue: nil, session: MoyaProvider<JobDetailAPI>.defaultAlamofireSession(), plugins: [RequestLoadingPlugin(), NetworkLogger()], trackInflights: false)

    func jobType(detailID value: Int) -> Driver<[JobDetailResponseSection]> {
        let params: [String: Any] = ["userId": 100_031, "coachDetailId": value]
        return provider.requestJson(.detail(value: params), isCache: false)
            .mapObject(type: JobDetailResponseModel.self).flatMap { (response) -> Observable<[JobDetailResponseSection]> in
                ...
                return Observable.just([JobDetailResponseSection(items: jobDetailList)])
