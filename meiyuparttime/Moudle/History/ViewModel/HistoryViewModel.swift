...
final class HistoryViewModel {
    ...
    /// 网络服务
    private var _network_service: HistoryNetworkService? = nil
    ...
    init(input headerRefresh: Driver<Void>, dependency: (
        ...
        let headerRefreshData = headerRefresh
            ...
            .flatMapLatest{  //也可考虑使用flatMapFirst
                return dependency.networkService.collectionInfo()
            }
    ...
    func load() {
        ...
        let headerRefreshData = _header_refresh!
            .startWith(()) //初始化时会先自动加载一次数据
            .flatMapLatest{  //也可考虑使用flatMapFirst
                return self._network_service!.collectionInfo()
            }
...
class HistoryNetworkService {
    ...
    func collectionInfo() -> Driver<[JobInfoListSection]> {
        ...
        return Observable.just([JobInfoListSection.init(items: response)]).asDriver(onErrorDriveWith: Driver.empty())
