...
class JobListController: ScrollingNavigationViewController, ScrollingNavigationControllerDelegate {
    ...
    private var right_item: UIButton = UIButton()
    ...
    /// 职位类型视图
    var jobTypeContentView: JobTypeContentView = JobTypeContentView.instance()!
    /// 尾部视图
    var jobContentFooterView: JobContentFooterView = JobContentFooterView.instance()!
    /// 空数据视图
    var jobContentEmptyView: JobContentEmptyView = JobContentEmptyView.instance()!
    
    var right_item_button: UIButton = UIButton()
    ...
    override func viewDidLoad() {
        ...
        contentTableView.rx.itemSelected.bind { [weak self] indexPath in
            ...
            let jobDetailController: JobDetailController = JobDetailController(jobInfo: jobInfoModel)
...
private extension JobListController {
    ...
    func load() {
        let dataSource =  RxTableViewSectionedReloadDataSource<JobInfoListSection>(configureCell: { ds, tv, ip, item in
