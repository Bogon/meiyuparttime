...
final class JobListViewModel {
    ...
    // 停止头部刷新状态
    var endHeaderRefreshing: Driver<Bool>? = nil
    
    // 停止尾部刷新状态
    var endFooterRefreshing: Driver<Bool>? = nil
    
    // 头部刷新
    private var _header_refresh: Driver<Void>? = nil
    
    // 尾部刷新
    private var _footer_refresh: Driver<Void>? = nil
    
    /// 网络服务
    private var _network_service: JobListNetworkService? = nil
    
    /// 包管理器
    private var _dispose_bag: DisposeBag? = nil
    ...
    init(input: (
        ...
        let headerRefreshData = _header_refresh!
            ...
            .flatMapLatest{ [weak self] in //也可考虑使用flatMapFirst
                return self!._network_service!.jobList(word_id: self!._work_id, city_name: self!._city_name, type: self!._type, vos: self!._vos)
            }
        
        // 上拉结果序列
         let footerRefreshData = _footer_refresh!
            .flatMapLatest{ [weak self] in //也可考虑使用flatMapFirst
                return self!._network_service!.nextPage(word_id: self!._work_id, city_name: self!._city_name, type: self!._type, vos: self!._vos)
            }
...
class JobListNetworkService {
    ...
    private let provider:MoyaProvider = MoyaProvider<JobListAPI>.init(endpointClosure: MoyaProvider<JobListAPI>.defaultEndpointMapping, requestClosure: MoyaProvider<JobListAPI>.defaultRequestMapping, stubClosure: MoyaProvider<JobListAPI>.neverStub, callbackQueue: nil, session:  MoyaProvider<JobListAPI>.defaultAlamofireSession(), plugins: [RequestLoadingPlugin(), NetworkLogger()], trackInflights: false)
    ...
    func jobList(word_id _id: String, city_name: String, type: String, vos: Int) -> Driver<[JobInfoListSection]> {
        ...
        return provider.requestJson(.jobList(value: params), isCache: false)
            .mapObject(type:JobListResponseModel.self).flatMap({ (response) -> Observable<[JobInfoListSection]> in
                ...
                return Observable.just([JobInfoListSection.init(items: jobInfoList)])
