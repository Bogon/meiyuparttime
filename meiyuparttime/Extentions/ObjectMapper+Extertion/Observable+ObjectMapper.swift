            ...
            let cancellableToken = self.request(token) { result in
                ...
                case let .success(response):
                    ...
                    if isCache == true, response.statusCode >= 200, response.statusCode <= 299 {
                        let cacheObj = Caccc(statusCode: response.statusCode, data: response.data, request: response.request, response: response.response)
                        // cache[key] = cacheObj
                ...
                case let .failure(error):
                    ...
                    if isCache == true, let res = cache[key] {
                        if res.statusCode >= 200, res.statusCode <= 299 {
                            let response = Response(statusCode: res.statusCode, data: res.data, request: res.request, response: res.response as! HTTPURLResponse)
                            observer.onNext(response)
                        }
                    } else {
